...
export default function Calculador(){
    ...
    const calcular = () => {
        ...
        if(integrantes.length < 2){
            ...
            let cadaUno = Math.round(fullTotal/integrantes.length)
            {integrantes.map((person) => (
                person.balance = Math.round(person.cuanto - cadaUno)
            ))}
            setCadaUno(cadaUno);
            setTotal(fullTotal);
    ...
    const calculateBalances = () => {
        ...
        let newArray = [];
        integrantes.map((person) => {
            const diferendo = Math.round(person.cuanto - promedio);
            newArray.push({
        ...
        debtors.forEach((debtor) => {
          ...
          creditors.forEach((creditor) => {
            if (amountOwed > 0 && creditor.balance > 0) {
              const payment = Math.min(amountOwed, creditor.balance);
    ...
    return (
        <form className="mt-14" onSubmit={handleSubmit}>
            {!calculado ?
                <>
                    <h1 id="subTitulo" className="mt-3 mb-5 text-2xl text-center mb-10">¿Cuánto puso cada uno/a?</h1>
                    <div className="grid gap-6 mb-6 space-x-4 md:grid-cols-6">
                        <div className="col-span-3 ...">
                            <input type="text" name="nombre" autoComplete="off" placeholder="Nombre" value={nombre} onChange={(e) => setNombre(e.target.value)} className="bg-white shadow-lg shadow-2xl text-gray-900 text-lg rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-grey dark:focus:ring-gray-100 dark:focus:border-gray-100" required />
                        </div>
                        <div className="col-span-2 ...">
                            <input type="number" min="0" placeholder="¿Cuánto puso?" value={cuanto > 0 && cuanto} onChange={(e) => setCuanto(parseInt(e.target.value))} name="cuanto" className="bg-white shadow-lg shadow-2xl text-gray-900 text-lg rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-grey dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                        </div>
                        <div>
            ...
            :
                <div className="flex flex-wrap flex-col items-center">
                    <div className="w-full block sm:flex flex-row justify-items-center mb-10">
                        {
                        integrantes.map((integrante, index) => (
                            <div key={index} className="flex flex-col sm:flex-row items-center justify-between m-2 px-4 py-2 bg-white shadow-lg shadow-2xl text-gray-900 text-base rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-grey">
                    ...
                    <span className="text-2xl sm:text-4xl">En total gastaron ${total}</span>
                    <span className="text-4xl sm:text-4xl mt-6 mb-14">(${cadaUno} cada uno/a)</span>

                    <span className="text-2xl mb-4 mt-2">Ajustes de Saldos</span>
